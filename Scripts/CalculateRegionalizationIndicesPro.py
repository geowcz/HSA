...
temp_fld = "Num_D" + inputNIDField
fieldlist = arcpy.ListFields(outputHSAs)
for field in fieldlist:
	if field.name == inputPopField:
		arcpy.DeleteField_management(outputHSAs, inputPopField)
	if field.name == "LI":
		arcpy.DeleteField_management(outputHSAs, "LI")
	if field.name == "EstTime":
		arcpy.DeleteField_management(outputHSAs, "EstTime")
	if field.name == "Compactness":
		arcpy.DeleteField_management(outputHSAs, "Compactness")	
	if field.name == temp_fld:
		arcpy.DeleteField_management(outputHSAs, temp_fld)
	if field.name == newCountNIDField:
		arcpy.DeleteField_management(outputHSAs, newCountNIDField)
