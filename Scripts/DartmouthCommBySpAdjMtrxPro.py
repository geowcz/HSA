        ...
        if com_node_li < li_limit:
            ...
            if isloop is False:
                if len(comm_neighborcom_mxwght) == 1:
                    ...
                    com_li_list_sort = dict(sorted(com_li_dict.items(), key = lambda value: value[1]))
                    com_li_list_coms = {key : value for (key, value) in com_li_list_sort.items() if value >= li_limit} 
                    if len(com_li_list_coms) == 0:
                        best_com = list(com_li_list_sort.keys())[-1]
                    else:
                        best_com = [key for (key, value) in com_li_list_coms.items() if value == min(com_li_list_coms.values())][0]
                else:
                    pass
